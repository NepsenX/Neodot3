...
<div class="response-image">
    ...
    async function sendMessage() {
        ...
        try {
            ...
            const formattedAiResponse = formatMarkdown(aiResponseContent);
            fullResponseHTML += `
...
<div class="video-item">
    ...
    async function addMessage(role, content, isHistory = false) { // Made this function async
        ...
        // Handle content display based on role and whether it's a new message or from history.
            ...
            if (content.includes('class="ai-response"')) {
                // It's already formatted HTML from sendMessage
                finalHTML = content;
            } else {
                // It's raw markdown (e.g., from web search)
                finalHTML = formatMarkdown(content);
            }

            // Extract plain text for typing effect
            const tempDivForParsing = document.createElement('div');
            tempDivForParsing.innerHTML = finalHTML;
            
            // Try to get text from .ai-response first, otherwise get all text
            const aiResponseElement = tempDivForParsing.querySelector('.ai-response');
            if (aiResponseElement) {
                aiTextContent = aiResponseElement.textContent; 
            } else {
                aiTextContent = tempDivForParsing.textContent;
            }
    ...
    function openVoiceMode() {
        ...
        voiceModeIframe.src = './src/voice.html';
